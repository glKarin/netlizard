...
void SimpleControlComponent::Reset()
{
    ...
    m_moveSens = settings->GetSetting<int>("CONTROL_3D/move_sens", M_Move_Sens);
    m_turnSens = settings->GetSetting<int>("CONTROL_3D/turn_sens", M_Turn_Sens);
    m_freelookSens = settings->GetSetting<double>("CONTROL_3D/freelook_sens", M_Freelook_Sens);
    NLComponent::Reset();
}
