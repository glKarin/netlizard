...
#define NLGET_LUA_OBJECT(L, T, name, index) \
    T *name = lua_isuserdata(L, index) ? *((T **)(lua_touserdata(L, index))) : 0

#define NLGET_LUA_OBJECT_USERDATA(L, T, name, index) \
    T **name = lua_isuserdata(L, index) ? (T **)(lua_touserdata(L, index)) : 0

#define NLNLGET_LUA_CALLER(L, T, name) NLGET_LUA_OBJECT(L, T, name, 1)

#define NLNLGET_LUA_CALLER_USERDATA(L, T, name) NLGET_LUA_OBJECT_USERDATA(L, T, name, 1)

#define NLPUSH_NLOBJECT_TO_STACK(L, T, data) {\
...
#define NLSET_GLOBAL_CFUNC(L, func) {\
    lua_pushcfunction(L, func); \
    lua_setglobal(L, #func); \
}

#define NLNULL_luaL_Reg {NULL, NULL}
