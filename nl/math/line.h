...
NLMATHLIB_EXPORT void line_make(line_t *line, const vector3_t *a, const vector3_t *b);
NLMATHLIB_EXPORT void line_direction(const line_t *line, vector3_t *dir);
NLMATHLIB_EXPORT int line_equals(const line_t *a, const line_t *b);
NLMATHLIB_EXPORT int line_equals_ignore_seq(const line_t *a, const line_t *b);
NLMATHLIB_EXPORT void line_ray_to_line(line_t *r, const ray_t *a, float length);
NLMATHLIB_EXPORT float line_length(const line_t *line);
NLMATHLIB_EXPORT void line_invertv(line_t *line);
NLMATHLIB_EXPORT int line_iszero(const line_t *line);

NLMATHLIB_EXPORT void ray_make(ray_t *line, const vector3_t *pos, const vector3_t *dir);
NLMATHLIB_EXPORT void ray_line_to_ray(ray_t *r, const line_t *l);
NLMATHLIB_EXPORT int ray_to_plane_intersect(const ray_t *line, const struct plane_s *plane, vector3_t *point);
NLMATHLIB_EXPORT int ray_to_plane_collision(const ray_t *line, const struct plane_s *plane, float *lamda, vector3_t *normal);
